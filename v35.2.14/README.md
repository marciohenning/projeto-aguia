...
## V35.2.13 — Correção mínima segura

Esta versão volta para a base original funcional da V35.2 enviada pelo usuário.
...
Alterações feitas:
- banco interno de perguntas atualizado para a base V35.7;
- pasta `data/perguntas` atualizada;
- Geometria adicionada ao banco e ao desempenho;
- seleção de 10, 20, 30, 50 ou 100 perguntas;
- conteúdos por disciplina atualizados.


---

## V35.2.14 — Correção do botão Entrar

Correção específica do login.

O botão `Entrar` agora usa `document.getElementById()` em vez de depender de variáveis globais do navegador.
